...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        console.log(`[auth] ${nid} 완료 → /study redirect +${Date.now() - t0}ms`)

        // ── /study로 직접 redirect (로딩 페이지 제거) ──
        const baseUrl = request.nextUrl.origin
        return NextResponse.redirect(new URL('/study', baseUrl), {
            status: 303,
        })
...
function errorRedirect(message: string, redirectUrl: string) {
